...
4. Verificar que la release contiene `update.json` y las firmas `.sig` de los artifacts de updater.
...
El JSON incluye `version`, `notes`, `pub_date` y `platforms` con `url` y `signature` para los assets encontrados.
