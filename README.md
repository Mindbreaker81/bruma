...
> [!IMPORTANT]
> La clave de firma del actualizador se rotó para la versión 1.8.0. Las
> instalaciones 1.7.x confían en la clave anterior y, por tanto, deben
> **descargar e instalar manualmente Bruma 1.8.0 una sola vez**. A partir de
> 1.8.0, las siguientes actualizaciones automáticas volverán a funcionar con
> normalidad.

El workflow `Release` genera artifacts de updater firmados para macOS ARM64, Linux x64, Windows x64 y Windows ARM64, y publica `update.json` con `scripts/generate-update-json.mjs`. La clave privada de updater no debe entrar al repo; en CI se usa `TAURI_SIGNING_PRIVATE_KEY` y opcionalmente `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`.
...
WebKitGTK 2.52+ puede mostrar una **ventana en blanco** (solo se ve la barra de menú y el área de contenido vacía) en Ubuntu 24.04 con GPU real, especialmente con drivers NVIDIA. Esto se debe al renderer DMABUF y al compositing acelerado. Bruma aplica automáticamente al iniciar en Linux:

- `WEBKIT_DISABLE_DMABUF_RENDERER=1`
- `WEBKIT_DISABLE_COMPOSITING_MODE=1`
- `__NV_DISABLE_EXPLICIT_SYNC=1` (mitiga errores Wayland/NVIDIA)

En el frontend, Linux Tauri también desactiva `backdrop-filter` y las capas decorativas con blur, porque WebKitGTK no las compone bien con esos workarounds y deja la UI invisible aunque React sí monte.

Ambas variables WebKit respetan el valor si ya las has definido (`WEBKIT_DISABLE_COMPOSITING_MODE=0` lo sobreescribe). Si la ventana sigue en blanco, prueba a lanzar la app con las variables explícitas:

```bash
WEBKIT_DISABLE_DMABUF_RENDERER=1 WEBKIT_DISABLE_COMPOSITING_MODE=1 __NV_DISABLE_EXPLICIT_SYNC=1 ./bruma-vX.Y.Z-linux-x86_64.AppImage
```
